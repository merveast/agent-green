#!/usr/bin/env python3
"""
Run two-agent log parsing experiment.
This script runs a generator agent and a critic agent to produce and
validate log templates. It uses CodeCarbon's OfflineEmissionsTracker to
...
def run_two_agent_inference_with_emissions_log_parsing(logs, llm_config, sys_prompt_log_parser, sys_prompt_critic, task_prompt, exp_name, result_dir):
    """
    Run the two-agent log parsing loop while tracking emissions.
    Returns the critic-corrected templates list.
    """
    gen_log_templates = []
...
def main():
    ...
    # Determine DESIGN with DA- prefix by default for two-agent runs unless explicitly provided
    design = args.design
    if design is None:
        DESIGN = f"DA-{shot}"
    else:
    ...
    time.sleep(5)
    try:
        last_templates = run_two_agent_inference_with_emissions_log_parsing(logs, llm_config, sys_prompt_generator, sys_prompt_critic, config.TASK_PROMPT_LOG_PARSING, exp_name, RESULT_DIR)
